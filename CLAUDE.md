...
- Per-occurrence ids: `<pid>-<yyyy-MM-dd>-<HH:mm>-primary`, `-followup` (one-shot) or `-followup-N` (the `repeatRemindersUntilDone` series, capped at `maxRepeatFollowUps`, kept within the dose's own day). `Prescription.followUpEnabled == false` schedules the primary only. All of a medication's requests share `threadIdentifier = prescriptionId` so they collapse into one group in Notification Center.
